...
def optimize_txt2img_prompt_only(prompt: str):
    if not prompt or not prompt.strip():
        return prompt, "Prompt is empty."

    base = prompt.strip()
    optimized = prompt_optimizer_llm.optimize_prompt_llm(base).strip()

    if optimized and base.lower() not in optimized.lower():
        new_prompt = f"{base}, {optimized}"
    else:
        new_prompt = optimized or base

    return new_prompt, ""
    

def generate_txt2img_negative_only(prompt: str, current_negative: str):
    if not prompt or not prompt.strip():
        return current_negative, "Prompt is empty."

    base = prompt.strip()
    generated_negative = prompt_optimizer_llm.generate_negative_prompt(base)

    if current_negative and current_negative.strip():
        new_negative = current_negative.strip() + ", " + generated_negative
    else:
        new_negative = generated_negative
...
def create_ui():
            ...
            with ExitStack() as stack:
                ...
                for category in ordered_ui_categories():
                    if category == "prompt":
                        ...
                        with FormRow(elem_id="txt2img_prompt_ai_row", variant="compact"):
                            txt2img_optimize_btn = gr.Button(
                                "AI Optimize Prompt",
                                elem_id="txt2img_optimize_prompt_btn",
                                elem_classes=["ai-prompt-button"],
                            )
                            txt2img_negative_btn = gr.Button(
                                "AI Generate Negative",
                                elem_id="txt2img_generate_negative_btn",
                                elem_classes=["ai-prompt-button"],
                            )

                        prompt_ai_message = gr.HTML(
                            "",
                            elem_id="txt2img_prompt_ai_message",
                        )

                        txt2img_optimize_btn.click(
                            fn=optimize_txt2img_prompt_only,
                            inputs=[toprow.prompt],
                            outputs=[toprow.prompt, prompt_ai_message],
                        )

                        txt2img_negative_btn.click(
                            fn=generate_txt2img_negative_only,
                            inputs=[toprow.prompt, toprow.negative_prompt],
                            outputs=[toprow.negative_prompt, prompt_ai_message],
                        )
            ...
            with ExitStack() as stack:
                ...
                for category in ordered_ui_categories():
                    if category == "prompt":
                        ...
                        with FormRow(elem_id="img2img_prompt_ai_row", variant="compact"):
                            img2img_optimize_btn = gr.Button(
                                "AI Optimize Prompt",
                                elem_id="img2img_optimize_prompt_btn",
                                elem_classes=["ai-prompt-button"],
                            )
                            img2img_negative_btn = gr.Button(
                                "AI Generate Negative",
                                elem_id="img2img_generate_negative_btn",
                                elem_classes=["ai-prompt-button"],
                            )

                        img2img_prompt_ai_message = gr.HTML(
                            "",
                            elem_id="img2img_prompt_ai_message",
                        )

                        def _optimize_img2img_prompt(user_prompt: str, current_prompt: str):
                            if not user_prompt.strip():
                                return current_prompt, "Prompt is empty."
                            optimized = prompt_optimizer_llm.optimize_prompt_llm(user_prompt)
                            return optimized, "Prompt optimized by AI."

                        def _generate_img2img_negative(user_prompt: str, current_negative: str):
                            if not user_prompt.strip():
                                return current_negative, "Prompt is empty."
                            negative = prompt_optimizer_llm.generate_negative_prompt(user_prompt)
                            return negative, "Negative prompt generated by AI."

                        img2img_optimize_btn.click(
                            fn=_optimize_img2img_prompt,
                            inputs=[toprow.prompt, toprow.prompt],
                            outputs=[toprow.prompt, img2img_prompt_ai_message],
                        )

                        img2img_negative_btn.click(
                            fn=_generate_img2img_negative,
                            inputs=[toprow.prompt, toprow.negative_prompt],
                            outputs=[toprow.negative_prompt, img2img_prompt_ai_message],
                        )
    ...
    with gr.Blocks(analytics_enabled=False) as image_retrieval_interface:
        ui_image_retrieval.create_ui()
